...
export class StartedWorkflow {
  ...
  public async cancel(reason?: string | undefined): Promise<void> {
    ...
    const handle = await this.client.client.startOperation(
      WorkflowService.operations.cancelWorkflow,
      RequestCancelWorkflowExecutionRequest.toProto(request as any) ?? {},
    );
    await handle.result();
  }
  ...
  public async restartWorkflow(
    ...
    const requestProto =
      StartWorkflowExecutionRequest.toProto(request as any) ?? {};
    const handle = await this.client.client.startOperation(
