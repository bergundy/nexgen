...
use crate::api_plan::{
    ...
    PlannedTypeInfo, PlannedValueType,
};
use crate::backend_common::{
    relative_descriptor_name, render_request_plan, service_client_name,
};
        ...
        return MessageValueConversion {
            ...
            kind: MessageValueConversionKind::Override {
                from_proto: language_override.from_proto.clone().unwrap_or_else(|| {
                    typescript_default_from_proto_name(&message.info.full_name)
                }),
                to_proto: language_override.to_proto.clone().unwrap_or_else(|| {
                    typescript_default_to_proto_name(&message.info.full_name)
                }),
            },
        };
    }
...
fn build_field(
    ...
    *uses_long |= resolved_type.uses_long;
    ...
    if matches!(field.role, PlannedFieldRole::Function(_)) {
        return RenderedField {
            name: generated_field_name.clone(),
            proto_name: proto_field_name.clone(),
                    ...
                    enum_conversion: enum_type.info.as_ref().map(|info| {
                        EnumValueConversion {
                            from_proto: replacement
                                ...
                                .clone()
                                .unwrap_or_else(|| {
                                    typescript_default_from_proto_name(&info.full_name)
                                }),
                            to_proto: replacement
                                .to_proto
                                .clone()
                                .unwrap_or_else(|| {
                                    typescript_default_to_proto_name(&info.full_name)
                                }),
                        }
                    }),
                }
...
fn render_module(
    ...
    if !resources.is_empty() {
        output.push('\n');
        for (resource, client_name) in resources {
    ...
    for method in &resource.methods {
        ...
        match &method.binding {
            PlannedResourceMethodBindingSpec::Operation {
                ...
                output.push_str(";\n");
                let operation_attr_name =
                    typescript_ident(&operation_name.to_lower_camel_case());
                if *direct_return {
                    if result_annotation == "void" {
                        output.push_str("    await this.client.");
                        output.push_str(&operation_attr_name);
                        output.push_str("(request);\n");
                    } else {
                        output.push_str("    return await this.client.");
                        output.push_str(&operation_attr_name);
                        output.push_str("(request);\n");
                    }
                } else {
                    output.push_str("    const handle = await this.client.");
                    output.push_str(&operation_attr_name);
                    output.push_str("(request);\n");
                    if result_annotation == "void" {
            ...
            if *hidden {
                ...
                if binding.optional {
                    ...
                    format!(
                        ...
                        typescript_string_literal(&typescript_generated_field_name(&binding.field_name)),
                    )
                }
            } else {
