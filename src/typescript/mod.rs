...
use crate::wit_symbols::{
    WitEnum, WitField, WitFieldKind, WitFlags, WitMessageSource, WitMessageType, WitOperation,
    WitOperationOutput, WitOperationResourceFieldBinding, WitOperationResourceReturn, WitResource,
    WitResourceMethod, WitResourceMethodBindingSpec, WitResourceMethodResultKind, WitScalarType,
    WitSourcedField, WitSymbols, WitTypeInfo, WitValueType, WitVariant, relative_descriptor_name,
};
            ...
            let operations = service
                ...
                .map(|operation| {
                    ...
                    rendered.input = Some(push_io_ref(
                        &mut io_type_refs,
                        rendered.input_proto_ref.clone(),
                    ));
                    rendered.output = Some(push_io_ref(
                        &mut io_type_refs,
                        rendered.output_proto_ref.clone(),
                    ));
                    Ok(rendered)
                })
...
fn resolve_operation<'a>(
    ...
    let (output_proto_ref, output_type_id, output_annotation_default) = match output {
        ...
        WitOperationOutput::None => ("void".to_string(), "void".to_string(), "void".to_string()),
    };
...
fn build_flattened_message_field(
    ...
    for nested_wit_field in nested_fields {
        let nested_rendered_field =
            build_field(&nested_wit_field, symbols, enums, flags, variants, models);
...
fn resolve_wit_value_type(
    ...
    match value_type {
        WitValueType::Scalar(WitScalarType::Float | WitScalarType::Int32) => ResolvedFieldType {
        ...
        WitValueType::Tuple(items) => {
            let item_types = items
                .iter()
                .map(|item| resolve_wit_value_type(item, symbols, enums, flags, variants, models))
                .collect::<Vec<_>>();
            let mut requirements = TypeScriptRequirements::default();
...
fn typescript_resource_type_id(service: &RenderedService<'_>, resource: &WitResource) -> String {
...
fn typescript_resource_value_annotation(value: &WitValueType) -> String {
    match value {
        WitValueType::Scalar(WitScalarType::Float | WitScalarType::Int32) => "number".to_string(),
...
fn resource_return_binding_expr_typescript(binding: &WitOperationResourceFieldBinding) -> String {
