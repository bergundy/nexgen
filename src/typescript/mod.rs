...
use crate::spec::{AuthoredFieldTypeSpec, FunctionResultSpec, TypeReplacementSpec};
...
fn generated_model_type_parameters(
    ...
    let mut type_parameters = generated_model
        .functions
        .iter()
        .filter(|(field_name, _)| !generated_model.with_arguments.contains_key(*field_name))
        .map(|(field_name, function)| {
            ...
            let constraint =
                function_constraint(&typescript_function_result_annotation(&function.result));
            (
                !function.primary,
...
fn build_field(
    ...
    if field.function_args || field.with_arguments_args {
        return RenderedField {
            name: generated_field_name.clone(),
            proto_name: proto_field_name.clone(),
...
fn render_resource(output: &mut String, resource: &PlannedResource, service: &RenderedService<'_>) {
    ...
    for method in &resource.methods {
        ...
        for param in &method.params {
            ...
            output.push_str(": ");
            output.push_str(&typescript_resource_field_annotation(&param.kind, false));
            output.push_str(",\n");
        }
...
fn typescript_resource_method_result_annotation(method: &PlannedResourceMethod) -> String {
    ...
    let annotation = match &result.kind {
        ...
        PlannedResourceMethodResultKind::Value(kind) => {
            typescript_resource_field_annotation(kind, result.optional)
        }
    };
...
fn typescript_resource_field_annotation(kind: &PlannedFieldKind, optional: bool) -> String {
    let base = match kind {
    ...
    method
        ...
        .filter(|param| {
            used_params.contains(&param.name)
                && typescript_resource_field_annotation(&param.kind, param.optional)
                    .trim_start()
                    .starts_with("string | ")
        })
