...
use crate::spec::{
    ApiSpec, Direction, OperationSpec, PythonTypeParameterKind, PythonTypeParameterSpec,
};
...
fn resolve_operation<'a>(
    ...
    let input_type_parameters = spec
        ...
        .map(|python_model| python_model.type_parameters.clone())
        .unwrap_or_default();
        ...
        .filter(|field| {
            ...
            !spec
                ...
                .is_some_and(|type_override| {
                    type_override.is_field_hidden(Language::Python, proto_name)
                })
        })
    ...
    let sourced_fields = message
        ...
        .filter_map(|field| {
            ...
            spec.type_override(&message.full_name)
                .and_then(|type_override| type_override.field_source(Language::Python, proto_name))
                .map(|source_expr| {
                    build_sourced_field(
                        message,
                        field,
...
fn message_model_annotation(full_name: &str, spec: &ApiSpec) -> String {
    ...
    match spec
        ...
        .and_then(|type_override| type_override.python_generated_model())
        .filter(|python_model| !python_model.type_parameters.is_empty())
    {
        Some(python_model) => {
            python_default_model_annotation(&model_name, &python_model.type_parameters)
        }
        None => model_name,
    }
}

fn python_parameterized_model_annotation(
    model_name: &str,
...
fn python_default_model_annotation(
    model_name: &str,
    type_parameters: &[PythonTypeParameterSpec],
) -> String {
    if type_parameters.is_empty() {
...
fn python_type_parameter_use(parameter: &PythonTypeParameterSpec) -> String {
    match parameter.kind {
        PythonTypeParameterKind::TypeVar => parameter.name.clone(),
        PythonTypeParameterKind::TypeVarTuple => format!("*{}", parameter.name),
    }
}

fn python_any_type_argument(parameter: &PythonTypeParameterSpec) -> String {
    match parameter.kind {
        PythonTypeParameterKind::TypeVar => "typing.Any".to_string(),
        PythonTypeParameterKind::TypeVarTuple => "*tuple[typing.Any, ...]".to_string(),
    }
}
