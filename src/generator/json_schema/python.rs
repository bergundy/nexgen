...
fn render_model(
    ...
    *needs_spec_int_helper |= schema_uses_integer(&schema);
    ...
    // `validate_assignment` keeps post-construction mutation under the same
    // constraints as construction (PRINCIPLES P10/P12) — without it
    // `model.field = <invalid>` passes locally and only fails at the wire.
    output.push_str(
        "    model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(strict=True, populate_by_name=True, validate_assignment=True, extra=",
    );
    ...
    for (json_name, property) in properties {
        ...
        if let Some(const_value) = &property.const_value {
            ...
            if !allows_null(property) {
                // Keyed on the Python attribute name: `field_validator` resolves
                // field names, not wire aliases, and covers both the parse and
                // the assignment path.
    ...
    *needs_optional_non_nullable_helper |= !optional_non_nullable_fields.is_empty();
    // The null check itself lives in the runtime helper, so the model body only
    // needs `pydantic_core` for the validators that build errors inline.
    *needs_pydantic_core |= !const_fields.is_empty()
...
fn render_optional_non_nullable_helper(output: &mut String) {
    output.push_str("def _reject_explicit_null(value: object) -> object:\n");
    output.push_str("    \"\"\"Rejects an explicit `null` on an optional, non-nullable field.\n\n");
    output
        .push_str("    Wired as a per-field after-validator, so it fires on both the parse path\n");
    output.push_str(
        "    and on assignment (`validate_assignment`). An omitted field never reaches\n",
    );
    output.push_str("    it — Pydantic does not validate defaults.\n");
    output.push_str("    \"\"\"\n");
...
fn render_optional_non_nullable_validator(output: &mut String, fields: &BTreeSet<String>) {
    ...
    output.push_str("mode=\"after\")\n");
    output.push_str("    @classmethod\n");
    output.push_str("    def _reject_null(cls, value: object) -> object:\n");
    output.push_str("        return _reject_explicit_null(value)\n");
}
