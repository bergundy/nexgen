...
use crate::spec::{GeneratedModelSpec, SupportFragmentSpec, TypeReplacementSpec};
...
fn generated_model_type_parameters(_generated_model: &GeneratedModelSpec) -> Vec<String> {
    Vec::new()
}
...
fn build_sourced_field(
    ...
    let source_expr = python_source_expr(&field.source_expr);
    ...
    let source_expr = python_sourced_value_expr(&resolved_type.annotation, &field.source_expr);
    if repeated {
        return RenderedSourcedField {
...
fn python_source_expr(source_expr: &str) -> String {
    source_expr.to_string()
}

fn python_sourced_value_expr(annotation: &str, source_expr: &str) -> String {
    let _ = annotation;
    python_source_expr(source_expr)
}
    ...
    match resolved_type.kind {
        ResolvedFieldKind::Message => lines.push(format!(
            ...
            resolved_type
                ...
                .as_ref()
                .expect("message conversion should be present")
                .to_proto_expr(&required_value_expr(attr_name, &resolved_type.annotation))
        )),
        ResolvedFieldKind::Enum => lines.push(format!(
            "message.{proto_name} = {}",
            enum_to_proto_expr(
                resolved_type,
                &required_value_expr(attr_name, &resolved_type.annotation)
            )
        )),
        _ => lines.push(format!(
            "message.{proto_name} = {}",
...
fn required_function_to_proto_lines(
    ...
    let converted_value = function_converter_expr(converter, &format!("self.{attr_name}"));
    match resolved_type.kind {
...
fn optional_to_proto_lines_from_expr(
    ...
    let typed_value_expr = optional_value_expr(value_expr, &resolved_type.annotation);
    match resolved_type.kind {
        ResolvedFieldKind::Message => vec![
        ...
        _ => vec![
            format!("if {value_expr} is not None:"),
            format!("    message.{proto_name} = {typed_value_expr}"),
        ],
    }
}

fn required_value_expr(attr_name: &str, annotation: &str) -> String {
    let _ = annotation;
    format!("self.{attr_name}")
}

fn optional_value_expr(value_expr: &str, annotation: &str) -> String {
    let _ = annotation;
...
fn render_resource_method_operation_body(
    ...
    output.push_str(&operation_request_construction_annotation(operation));
    output.push_str("(\n");
    match request_plan {
...
fn operation_request_construction_annotation(operation: &RenderedOperation<'_>) -> String {
    operation
        .unpacked_input
        .as_ref()
        .map(function_implementation_model_annotation)
        .unwrap_or_else(|| operation.input_annotation.clone())
}
    ...
    match &function.args {
        RenderedFunctionArgs::Unbounded { .. } => {
            ...
            cases.push(RenderedFunctionOverloadCase {
                ...
                args_optional: false,
                type_parameters: vec![first_arg.clone()],
            });
            cases.push(RenderedFunctionOverloadCase {
                callable_field_name: function.callable_field_name.clone(),
                args_field_name: function.args_field_name.clone(),
