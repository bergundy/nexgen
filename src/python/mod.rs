...
use crate::spec::{ApiSpec, Direction, GeneratedModelSpec, OperationSpec, TypeReplacementSpec};
...
pub(crate) fn resolve_message_ref<'a>(
    ...
    for candidate in &candidates {
        if let Some(message) = descriptors.message(candidate) {
            return Ok(message);
        }
    }

    let suffix = format!(".{type_name}");
    let mut matches = descriptors.messages_with_suffix(&suffix);
...
fn resolve_operation<'a>(
    ...
    let input_message = resolve_message_ref(descriptors, input_ref)?;
    let output_message = resolve_message_ref(descriptors, output_ref)?;
    ...
    let input_type_parameters = spec
    ...
    Ok(RenderedOperation {
        ...
        input_annotation: if input_type_parameters.is_empty() {
            ...
            python_parameterized_model_annotation(
                &message_model_name(&input_message.full_name),
...
fn required_python_ref<'a>(
    ...
    Ok(operation.reference(direction))
}
...
fn resolve_message_value_conversion(
        ...
        .and_then(|type_override| type_override.replacement())
    {
        return MessageValueConversion {
            ...
            kind: MessageValueConversionKind::Override {
                from_proto: python_from_proto_converter(&message.full_name, language_override),
                to_proto: python_to_proto_converter(&message.full_name, language_override),
            },
    ...
    MessageValueConversion {
        annotation: message_model_annotation(&message.full_name, spec),
...
fn build_field(
    ...
    let attr_name = spec
        ...
        .map(python_field_name)
        .unwrap_or_else(|| python_field_name(proto_name));
    let function_field = spec
        .type_override(&message.full_name)
        .and_then(|type_override| type_override.generated_model())
    ...
    if required {
        return RenderedField {
            ...
            to_proto_lines: match function_field {
                Some(function_field) if resolved_type.kind != ResolvedFieldKind::Message => {
                ...
                _ => required_to_proto_lines(&model_name, &attr_name, proto_name, &resolved_type),
            },
            imports: resolved_type.imports,
        };
    }
    ...
    RenderedField {
        ...
        to_proto_lines: match function_field {
            Some(function_field) if resolved_type.kind != ResolvedFieldKind::Message => {
                optional_function_to_proto_lines(proto_name, function_field.primary)
            }
            _ => optional_to_proto_lines(&resolved_type, proto_name),
        },
        imports: resolved_type.imports,
    }
}
...
fn build_sourced_field(
    ...
    RenderedSourcedField {
        to_proto_lines: sourced_optional_to_proto_lines(&resolved_type, source_expr, proto_name),
    }
}
...
fn python_field_annotation(
    ...
    if let Some(function) = python_model.function(field_name) {
        return function_annotation(&function.result_type);
    }

    if python_model.function_for_args_field(field_name).is_some() {
...
fn message_model_annotation(full_name: &str, spec: &ApiSpec) -> String {
    let model_name = message_model_name(full_name);
    match spec
        .type_override(full_name)
        .and_then(|type_override| type_override.generated_model())
        .map(generated_model_type_parameters)
        .filter(|type_parameters| !type_parameters.is_empty())
    {
        Some(type_parameters) => python_default_model_annotation(&model_name, &type_parameters),
        None => model_name,
    }
}
...
mod tests {
    ...
    fn resolves_sample_python_reference() {
        ...
        let message = resolve_message_ref(
            &descriptors,
