...
fn python_field_annotation(
    ...
    if matches!(field.role, PlannedFieldRole::FunctionArgs) {
        let args_annotation = "list[typing.Any]".to_string();
...
fn render_optional_python_imports(
    ...
    let symbol_imports = [("from datetime import timedelta\n", "timedelta")];
    let alias_imports = [("from temporalio import workflow\n", "workflow")];
    ...
    for (import_line, symbol) in symbol_imports {
        if body_uses_python_symbol(body, symbol) {
            output.push_str(import_line);
            wrote_any = true;
        }
    }
    for (import_line, alias) in alias_imports {
        if body_uses_python_module_path(body, alias) {
            output.push_str(import_line);
            wrote_any = true;
        }
    }
...
#[derive(Debug, Clone)]
struct RenderedFunctionOverloadPositionalArgs {
    name: String,
    annotation: String,
}
...
fn function_overload_cases(function: &RenderedFunctionField) -> Vec<RenderedFunctionOverloadCase> {
    let mut cases = vec![RenderedFunctionOverloadCase {
        ...
        positional_args: function
            .primary
            .then(|| RenderedFunctionOverloadPositionalArgs {
...
fn render_function_unpacked_overload(
    ...
    if let (Some(function), Some(function_case)) = (primary_function, primary_case) {
        ...
        output.push_str(",\n");
        if let Some(positional_args) = &function_case.positional_args {
            output.push_str("    ");
            output.push('*');
            output.push_str(&positional_args.name);
            output.push_str(": ");
            output.push_str(&positional_args.annotation);
            output.push_str(",\n");
        }
    }

    if !primary_case.is_some_and(|function_case| function_case.positional_args.is_some()) {
        output.push_str("    *,\n");
    }
    ...
    for field in &unpacked_input.parameters {
            ...
            .find(|function| function.args_field_name == field.attr_name)
        {
            let generated_doc = if function.primary {
                ...
                format!(
                    "Argument value, or list of argument values, for {}.",
                    function.callable_field_name
                )
            };
            args.push((
...
fn render_function_unpacked_implementation(
    ...
    for field in &unpacked_input.parameters {
            ...
            .any(|function| function.args_field_name == field.attr_name)
        {
            ...
            if primary_args {
                output.push_str("list[typing.Any] | None");
            } else {
                output.push_str("object | list[typing.Any] | None");
            }
        } else {
            output.push_str(&python_parameter_annotation(
                &field.annotation,
...
class Example(enum.Enum):
    ...
    def run(self) -> typing_extensions.Self:
        workflow.create_nexus_client(service="svc", endpoint="endpoint")
        return self
"#;

        let mut output = String::new();
        let wrote_imports =
            super::render_optional_python_imports(&mut output, body, &module_imports);
        ...
        assert!(output.contains("from datetime import timedelta\n"));
