...
use crate::spec::{
    AuthoredFieldTypeSpec, FunctionResultSpec, LanguageImportSpec, LanguageImportStyle,
            ...
            .find(|function| function.args_field_name == field.attr_name)
        {
            let generated_doc = if function.primary {
                format!(
                    ...
                    For typed workflow callables, list contents are not statically typechecked; \
                    pass workflow arguments positionally for precise typechecking.",
                    function.callable_field_name
                )
            } else {
                format!(
                    "Argument value, or list of argument values, for {}. For typed \
                    single-argument signals, scalar signal_args values are statically typechecked. \
                    List-form signal_args values are not precisely typechecked. To pass a single \
                    signal argument that is itself a list, wrap it in another list; otherwise the \
                    list is interpreted as multiple signal arguments.",
                    function.callable_field_name
                )
            };
            args.push((
