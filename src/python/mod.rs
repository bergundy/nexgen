...
use crate::spec::{
    AuthoredFieldTypeSpec, FunctionResultSpec, SupportFragmentSpec, TypeReplacementSpec,
};
...
fn render_package(
    ...
    insert_generated_file(
        &mut files,
        "models.py",
        render_models_module(enums, flags, variants, models, &support_names),
    )?;
    insert_generated_file(
        &mut files,
...
fn python_resource_method_result_annotation(method: &PlannedResourceMethod) -> String {
    ...
    let annotation = match &result.kind {
        ...
        PlannedResourceMethodResultKind::Value(kind) => {
            python_resource_field_annotation(kind, result.optional)
        }
    };
...
fn python_resource_field_annotation(kind: &PlannedFieldKind, optional: bool) -> String {
    let base = match kind {
...
fn render_inline_nexus_client(output: &mut String, service: &RenderedService<'_>, indent: &str) {
    output.push_str(indent);
    output.push_str("nexus_client = workflow.create_nexus_client(\n");
    output.push_str(indent);
    output.push_str("    service=");
