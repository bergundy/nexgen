...
/// A [`NameResolver`](nex_gen_core::NameResolver) that is never consulted.
///
/// The .NET front-end keeps every generated type in a single namespace, so its
/// files declare no cross-module `refs`; [`resolve_imports`](nex_gen_core::resolve_imports)
/// then only dedups the runtime `using` imports and never touches the resolver.
struct NoRefsResolver;

impl nex_gen_core::NameResolver for NoRefsResolver {
    fn type_ref(&self, id: nex_gen_core::SymbolId) -> String {
        unreachable!("dotnet declares no refs; resolver not consulted for {id:?}")
    }

    fn module_of(&self, id: nex_gen_core::SymbolId) -> nex_gen_core::Module {
        unreachable!("dotnet declares no refs; resolver not consulted for {id:?}")
    }

    fn import_binding(&self, id: nex_gen_core::SymbolId) -> nex_gen_core::Import {
        unreachable!("dotnet declares no refs; resolver not consulted for {id:?}")
    }
}
...
fn generated_file_prelude(namespace: &str, imports: &[&str]) -> String {
    use nex_gen_core::{Import, ImportBinding, Language as CoreLanguage, Module};
    ...
    // Route the `using` block through the core resolve + render helpers: each
    // namespace is a whole-module runtime import (a `using X;`). .NET keeps
    // every generated type in one namespace, so there are no cross-module
    // `refs` — `resolve_imports` just dedups, preserving the dependency order
    // the front-end supplies (`render_imports` for .NET does not re-sort).
    let runtime_imports: Vec<Import> = imports
        .iter()
        .map(|module| Import {
        ...
        .collect();
    // Empty file module: .NET has no same-module `using` exclusion — a file may
    // legitimately `using` its own namespace (e.g. an `Operations.cs` generated
    // into `Temporalio.Workflows`) — so use a sentinel that matches nothing.
    let file_module = Module::new("");
    let resolved = nex_gen_core::resolve_imports(&NoRefsResolver, &file_module, &[], &runtime_imports);
    let using_block = nex_gen_core::render_imports(CoreLanguage::Dotnet, &resolved);
