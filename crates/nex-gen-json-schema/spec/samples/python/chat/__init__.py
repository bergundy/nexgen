...
class Message(BaseModel):
    ...
    # Open-enum hint for the `const` discriminator (P13.1); the
    # validator closes it to "text".
    ...
    priority: Optional[SpecInt] = Field(default=0)
    """Delivery priority."""
    ...
    def _const_kind(cls, data):
        if isinstance(data, dict):
            ...
            elif data["kind"] != cls._KIND_CONST:
...
class Room(BaseModel):
    ...
    _OPTIONAL_NON_NULLABLE: ClassVar[frozenset] = frozenset({"members", "labels"})
...
class Labels(BaseModel):
    ...
    # Typed map -> named wrapper: extras live in `model_extra`, each
    # validated by type; count capped at maxProperties.
...
class GetRoomInput(BaseModel):
    # Synthesized from an inline operation object.
...
SendMessageOutput.model_rebuild()
Message.model_rebuild()
Room.model_rebuild()
Labels.model_rebuild()
GetRoomInput.model_rebuild()
...
class ChatService:
    ...
    send_message: Operation[SendMessageInput, SendMessageOutput] = Operation(name="SendMessage")
