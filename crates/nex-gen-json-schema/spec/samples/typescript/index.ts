...
/** Advisory default for `Message.priority`. Read via `?? DEFAULT_PRIORITY`. */
...
export function parseSendMessageInput(raw: unknown): SendMessageInput {
  ...
  if (raw.message === undefined || raw.message === null) {
    ...
    try { message = parseMessage(raw.message); } catch (e) { collect(violations, "message", e); }
  }

  // Closed struct: reject unknown keys.
  for (const k of Object.keys(raw)) {
    if (k !== "roomId" && k !== "message") violations.push({ path: k, reason: "unknown field" });
  }
...
export function parseMessage(raw: unknown): Message {
  ...
  // replyToId: optional + nullable string.
  let replyToId: string | null | undefined = undefined;
  if (raw.replyToId === null) { replyToId = null; }
  else if (raw.replyToId !== undefined) {
    if (typeof raw.replyToId !== "string") violations.push({ path: "replyToId", reason: "expected string" });
    else replyToId = raw.replyToId;
  }
  ...
  if (raw.priority === null) {
    ...
    if (typeof raw.priority !== "number" || !Number.isSafeInteger(raw.priority)) violations.push({ path: "priority", reason: "expected integer" });
    else priority = raw.priority;
  }

  // Closed struct: reject unknown keys.
  for (const k of Object.keys(raw)) {
    if (k !== "kind" && k !== "body" && k !== "replyToId" && k !== "priority") violations.push({ path: k, reason: "unknown field" });
  }
...
export function parseRoom(raw: unknown): Room {
  ...
  } else { displayName = raw.displayName; }

  // topic: required + nullable.
  ...
  if (raw.members === null) {
    ...
    if (!Array.isArray(raw.members)) {
      ...
      members = [];
      (raw.members as unknown[]).forEach((el, i) => {
        if (typeof el !== "string") {
          violations.push({ path: `members[${i}]`, reason: "expected element" });
        } else { members!.push(el as string); }
      });
    }
  }
  ...
  if (raw.labels === null) {
    ...
    try { labels = parseLabels(raw.labels); } catch (e) { collect(violations, "labels", e); }
  }
...
export function serializeRoom(value: Room): unknown {
  ...
  out.topic = value.topic; // required + nullable: always emitted
  if (value.members !== undefined) out.members = value.members;
  if (value.labels !== undefined) out.labels = serializeLabels(value.labels);
  for (const [k, v] of Object.entries(value.additionalProperties ?? {})) out[k] = v;
  return out;
}
...
export function parseLabels(raw: unknown): Labels {
  ...
  for (const k of keys) {
    ...
    } else { additionalProperties[k] = raw[k] as string; }
  }
  if (violations.length) throw new ValidationError(violations);
